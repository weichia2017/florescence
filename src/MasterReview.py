...
class Master:
    ...
    def adj_noun_pairs(self, num_of_noun=10, num_of_adj_each=3):
        ...
        def spcy(row):
            ...
            for token in doc:
                if token.pos_ == 'ADJ' and token.dep_ == "amod" and token.head != None and token.head.pos_ in ['PROPN', 'NOUN'] and token.text != "-":
                    new_row = {'review_id': row.review_id, 'noun': token.head.lemma_.lower(
        ...
        top_noun_adj_pair = pairs.groupby(['noun']).apply(lambda x: x.nlargest(num_of_adj_each, ['count'], keep='first')).reset_index(
